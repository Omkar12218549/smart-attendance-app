...
function clearData() {
  db.exec(`
    ...
    DELETE FROM users;
    DELETE FROM sqlite_sequence WHERE name IN ('users','subjects','classes','attendance','timetable');
  `);
}
...
async function seed() {
  ...
  for (let i = 7; i >= 1; i--) {
    ...
    slots.forEach(([sid, st, et, room], slotIdx) => {
      ...
      studentIds.forEach((stuId, sIdx) => {
        // ~85-90% present rate, with some absences for realism
        const r = Math.random();
  ...
  console.log('📊 Subjects:', { sub1, sub2, sub3 });
}
